use crate::types::arpa_chain_model_to_randomness_task;
...
use entity::prelude::ArpaChainRandomnessTask;
use sea_orm::{
...
impl SqliteDB {
    pub fn get_arpa_chain_bls_tasks_client<T: Task>(&self) -> ArpaChainBLSTasksDBClient<T> {
        ArpaChainBLSTasksDBClient {
...
#[derive(Debug, Clone)]
pub struct ArpaChainBLSTasksDBClient<T: Task> {
    db_client: Arc<SqliteDB>,
    bls_tasks: PhantomData<T>,
}

impl ArpaChainBLSTasksDBClient<RandomnessTask> {
...
#[async_trait]
impl BLSTasksFetcher<RandomnessTask> for ArpaChainBLSTasksDBClient<RandomnessTask> {
    async fn contains(&self, task_request_id: &[u8]) -> DataAccessResult<bool> {
        let conn = &self.db_client.connection;
        let task = ArpaChainRandomnessTaskQuery::select_by_request_id(conn, task_request_id)
            .await
            .map_err(|e| {
    ...
    async fn get(&self, task_request_id: &[u8]) -> DataAccessResult<RandomnessTask> {
        ...
        task.map(arpa_chain_model_to_randomness_task)
            .ok_or_else(|| {
...
#[async_trait]
impl BLSTasksUpdater<RandomnessTask> for ArpaChainBLSTasksDBClient<RandomnessTask> {
    async fn add(&mut self, task: RandomnessTask) -> DataAccessResult<()> {
        let seed_bytes = u256_to_vec(&task.seed);

        ArpaChainRandomnessTaskMutation::add_task(
            self.get_connection(),
            task.request_id,
    ...
    async fn check_and_get_available_tasks(
        ...
        ArpaChainRandomnessTaskMutation::fetch_available_tasks(
            self.get_connection(),
        ...
        .map(|models| {
            models
                .into_iter()
                .map(arpa_chain_model_to_randomness_task)
                .collect::<Vec<_>>()
        })
        .map_err(|e| {
            let e: DBError = e.into();
...
pub struct ArpaChainRandomnessTaskQuery;

impl ArpaChainRandomnessTaskQuery {
    pub async fn select_by_request_id(
        db: &DbConn,
        request_id: &[u8],
    ) -> Result<Option<arpa_chain_randomness_task::Model>, DbErr> {
        ArpaChainRandomnessTask::find()
            .filter(arpa_chain_randomness_task::Column::RequestId.eq(request_id))
            .one(db)
            .await
    }
}

pub struct ArpaChainRandomnessTaskMutation;

impl ArpaChainRandomnessTaskMutation {
    ...
    ) -> Result<arpa_chain_randomness_task::ActiveModel, DbErr> {
        arpa_chain_randomness_task::ActiveModel {
    ...
    ) -> Result<Vec<arpa_chain_randomness_task::Model>, DbErr> {
        arpa_chain_randomness_task::Model::find_by_statement(Statement::from_sql_and_values(
                DbBackend::Sqlite,
                r#"update arpa_chain_randomness_task set state = 1, update_at = $1 where state = 0 and (group_index = $2 or assignment_block_height < $3) 
                returning *"#,
