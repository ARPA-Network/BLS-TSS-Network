use crate::types::bsc_model_to_randomness_task;
...
use entity::prelude::BscRandomnessTask;
use sea_orm::{
...
impl SqliteDB {
    pub fn get_bsc_bls_tasks_client<T: Task>(&self) -> BSCBLSTasksDBClient<T> {
        BSCBLSTasksDBClient {
...
#[derive(Debug, Clone)]
pub struct BSCBLSTasksDBClient<T: Task> {
    db_client: Arc<SqliteDB>,
    bls_tasks: PhantomData<T>,
}

impl BSCBLSTasksDBClient<RandomnessTask> {
...
#[async_trait]
impl BLSTasksFetcher<RandomnessTask> for BSCBLSTasksDBClient<RandomnessTask> {
    async fn contains(&self, task_request_id: &[u8]) -> DataAccessResult<bool> {
        let conn = &self.db_client.connection;
        let task = BSCRandomnessTaskQuery::select_by_request_id(conn, task_request_id)
            .await
            .map_err(|e| {
    ...
    async fn get(&self, task_request_id: &[u8]) -> DataAccessResult<RandomnessTask> {
        ...
        task.map(bsc_model_to_randomness_task).ok_or_else(|| {
...
#[async_trait]
impl BLSTasksUpdater<RandomnessTask> for BSCBLSTasksDBClient<RandomnessTask> {
    async fn add(&mut self, task: RandomnessTask) -> DataAccessResult<()> {
        let seed_bytes = u256_to_vec(&task.seed);

        BSCRandomnessTaskMutation::add_task(
            self.get_connection(),
            task.request_id,
    ...
    async fn check_and_get_available_tasks(
        ...
        BSCRandomnessTaskMutation::fetch_available_tasks(
            self.get_connection(),
        ...
        .map(|models| {
            models
                .into_iter()
                .map(bsc_model_to_randomness_task)
                .collect::<Vec<_>>()
        })
        .map_err(|e| {
            let e: DBError = e.into();
...
pub struct BSCRandomnessTaskQuery;

impl BSCRandomnessTaskQuery {
    pub async fn select_by_request_id(
        db: &DbConn,
        request_id: &[u8],
    ) -> Result<Option<bsc_randomness_task::Model>, DbErr> {
        BscRandomnessTask::find()
            .filter(bsc_randomness_task::Column::RequestId.eq(request_id))
            .one(db)
            .await
    }
}

pub struct BSCRandomnessTaskMutation;

impl BSCRandomnessTaskMutation {
    ...
    ) -> Result<bsc_randomness_task::ActiveModel, DbErr> {
        bsc_randomness_task::ActiveModel {
    ...
    ) -> Result<Vec<bsc_randomness_task::Model>, DbErr> {
        bsc_randomness_task::Model::find_by_statement(Statement::from_sql_and_values(
                DbBackend::Sqlite,
                r#"update bsc_randomness_task set state = 1, update_at = $1 where state = 0 and (group_index = $2 or assignment_block_height < $3) 
                returning *"#,
