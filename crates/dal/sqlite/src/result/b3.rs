...
use entity::b3_randomness_result;
use entity::prelude::B3RandomnessResult;
use migration::Expr;
use migration::Query;
use migration::SelectStatement;
use migration::SimpleExpr;
use migration::{
    B3RandomnessResult as B3RandomnessResultTable, B3RandomnessTask as B3RandomnessTaskTable,
};
...
impl SqliteDB {
    pub async fn get_b3_randomness_result_client(
        &self,
    ) -> DataAccessResult<B3SignatureResultDBClient<RandomnessResultCache>> {
        ...
        let update_stmt = Query::update()
            .table(B3RandomnessResultTable::Table)
            .values([
                (
                    B3RandomnessResultTable::State,
                    BLSResultCacheState::NotCommitted.to_i32().into(),
                ),
                (B3RandomnessResultTable::UpdateAt, format_now_date().into()),
            ])
            .and_where(
                Expr::col(B3RandomnessResultTable::State)
...
#[derive(Debug, Clone)]
pub struct B3SignatureResultDBClient<C: ResultCache> {
    db_client: Arc<SqliteDB>,
    signature_results_cache: InMemorySignatureResultCache<C>,
}

impl B3SignatureResultDBClient<RandomnessResultCache> {
    ...
    for B3SignatureResultDBClient<RandomnessResultCache>
{
    async fn contains(&self, task_request_id: &[u8]) -> DataAccessResult<bool> {
        let model =
            B3RandomnessResultQuery::select_by_request_id(self.get_connection(), task_request_id)
                .await
                .map_err(|e| {
                    let e: DBError = e.into();
                    e
    ...
    async fn get(
        ...
        let query_stmt = build_randomness_record_query(Some(
            Expr::col((
                B3RandomnessResultTable::Table,
                B3RandomnessResultTable::RequestId,
            ))
            .eq(task_request_id),
        ));
    ...
    for B3SignatureResultDBClient<RandomnessResultCache>
{
    async fn get_ready_to_commit_signatures(
        ...
        let update_stmt = Query::update()
            ...
            .and_where(Expr::col(B3RandomnessResultTable::RequestId).is_in(request_ids))
            .to_owned();
    ...
    async fn update_commit_result(
        ...
        let update_stmt = Query::update()
            .table(B3RandomnessResultTable::Table)
            .values([
                (B3RandomnessResultTable::State, status.to_i32().into()),
                (B3RandomnessResultTable::UpdateAt, format_now_date().into()),
            ])
            .and_where(Expr::col(B3RandomnessResultTable::RequestId).eq(task_request_id))
            .to_owned();
    ...
    async fn add(
        ...
        B3RandomnessResultMutation::add(
            self.get_connection(),
    ...
    async fn add_partial_signature(
        ...
        B3RandomnessResultMutation::add_partial_signature(
            self.get_connection(),
            model,
            member_address,
    ...
    async fn incr_committed_times(&mut self, task_request_id: &[u8]) -> DataAccessResult<()> {
        let update_stmt = Query::update()
            ...
            .values([
                (
                    B3RandomnessResultTable::CommittedTimes,
                    Expr::col(B3RandomnessResultTable::CommittedTimes).add(1),
                ),
                (B3RandomnessResultTable::UpdateAt, format_now_date().into()),
            ])
            .and_where(Expr::col(B3RandomnessResultTable::RequestId).eq(task_request_id))
...
pub struct B3RandomnessResultQuery;

impl B3RandomnessResultQuery {
    pub async fn select_by_request_id(
        db: &DbConn,
        request_id: &[u8],
    ) -> Result<Option<b3_randomness_result::Model>, DbErr> {
        B3RandomnessResult::find()
            .filter(b3_randomness_result::Column::RequestId.eq(request_id))
            .one(db)
            .await
    }
}

pub struct B3RandomnessResultMutation;

impl B3RandomnessResultMutation {
    pub async fn add(
        db: &DbConn,
        request_id: Vec<u8>,
        group_index: i32,
        message: Vec<u8>,
        threshold: i32,
    ) -> Result<b3_randomness_result::ActiveModel, DbErr> {
        b3_randomness_result::ActiveModel {
    ...
    pub async fn add_partial_signature(
        db: &DbConn,
        model: b3_randomness_result::Model,
        member_address: Address,
        partial_signature: Vec<u8>,
    ) -> Result<b3_randomness_result::Model, DbErr> {
        ...
        let mut randomness_result: b3_randomness_result::ActiveModel = model.into();
...
pub(crate) fn build_randomness_record_query(and_where: Option<SimpleExpr>) -> SelectStatement {
    Query::select()
        ...
        .column((
            B3RandomnessResultTable::Table,
            B3RandomnessResultTable::GroupIndex,
        ))
        .column((
            B3RandomnessResultTable::Table,
            B3RandomnessResultTable::Message,
        ))
        .column((
            B3RandomnessResultTable::Table,
            B3RandomnessResultTable::Threshold,
        ))
        .column((
            B3RandomnessResultTable::Table,
            B3RandomnessResultTable::PartialSignatures,
        ))
        .column((
            B3RandomnessResultTable::Table,
            B3RandomnessResultTable::CommittedTimes,
        ...
        .column((
            B3RandomnessTaskTable::Table,
            B3RandomnessTaskTable::SubscriptionId,
        ))
        .column((
            B3RandomnessTaskTable::Table,
            B3RandomnessTaskTable::RequestType,
        ))
        .column((B3RandomnessTaskTable::Table, B3RandomnessTaskTable::Params))
        .column((
            B3RandomnessTaskTable::Table,
            B3RandomnessTaskTable::Requester,
        ))
        .column((B3RandomnessTaskTable::Table, B3RandomnessTaskTable::Seed))
        .column((
            B3RandomnessTaskTable::Table,
            B3RandomnessTaskTable::RequestConfirmations,
        ))
        .column((
            B3RandomnessTaskTable::Table,
            B3RandomnessTaskTable::CallbackGasLimit,
        ))
        .column((
            B3RandomnessTaskTable::Table,
            B3RandomnessTaskTable::CallbackMaxGasPrice,
        ))
        .column((
            B3RandomnessTaskTable::Table,
            B3RandomnessTaskTable::AssignmentBlockHeight,
        ))
        .from(B3RandomnessResultTable::Table)
        .inner_join(
            B3RandomnessTaskTable::Table,
            Expr::col((
                B3RandomnessResultTable::Table,
            ...
            .equals((
                B3RandomnessTaskTable::Table,
                B3RandomnessTaskTable::RequestId,
            )),
        )
        .conditions(
            and_where.is_some(),
