...
mod tests {
    ...
    impl TestEnvironment {
        async fn new() -> NodeResult<Self> {
            ...
            let chain_id_test = http_provider.get_chainid().await
            ...
            let ws_provider = Arc::new(
                Provider::<Ws>::connect(&anvil.ws_endpoint()).await
                    .map_err(|e| anyhow!("Failed to connect to WebSocket: {}", e))?
            );
            println!("Connected to Anvil WebSocket at {}", anvil.ws_endpoint());
            
            let ws_chain_id = ws_provider.get_chainid().await
        ...
        async fn setup_event_subscription(&self) -> tokio::sync::mpsc::Receiver<Box<dyn std::any::Any + Send>> {
        ...
        async fn emit_test_event(&self, params: &TestEventParams) -> NodeResult<u64> {
            ...
            let mock_adapter = deploy_and_get_mock_adapter(self.client.clone()).await
        ...
        async fn get_current_block_number(&self) -> NodeResult<u64> {
            let current_block = self.ws_provider.get_block(BlockNumber::Latest).await?
    ...
    impl TestEventParams {
        fn verify_against_event(&self, event: &NewRandomnessTask, expected_chain_id: usize, _current_block_number: u64) -> NodeResult<()> {
            ...
            assert_eq!(event.randomness_task.request_confirmations, self.request_confirmations);
            assert_eq!(event.randomness_task.callback_gas_limit, self.callback_gas_limit);
            assert_eq!(event.randomness_task.callback_max_gas_price, self.callback_max_gas_price);
        ...
        async fn verify_in_cache(&self, cache: &Arc<RwLock<Box<dyn BLSTasksHandler<RandomnessTask>>>>) -> NodeResult<()> {
    ...
    async fn test_new_randomness_task_listener() -> NodeResult<()> {
        ...
        test_params.verify_in_cache(&env.randomness_tasks_cache).await?;
