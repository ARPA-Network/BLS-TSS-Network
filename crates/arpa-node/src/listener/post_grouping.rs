...
mod tests {
    ...
    use arpa_core::{
        Config, DKGStatus, FixedIntervalRetryDescriptor, GeneralMainChainIdentity, Group, ListenerType
    };
    use arpa_dal::{
    ...
    async fn mock_subscribe_to_events(
        ...
        impl Subscriber for TestSubscriber {
            async fn notify(&self, _topic: Topic, payload: &dyn DebuggableEvent) -> NodeResult<()> {
                if let Some(post_process_event) = payload.as_any().downcast_ref::<DKGPostProcess<G2Curve>>() {
